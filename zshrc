...
# IP1=$(ip -4 addr | grep -v 127.0.0.1 | grep -v secondary | grep 'eth0\|eth1\|eth2' |grep -Po "inet \K[\d.]+" | awk '{ORS=(NR%3?FS="/":RS)}1') # Get normal interface, may need to be changed
...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$'%m%b%F{%(#.blue.green)})-[%B%F{yellow}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]'$LOCAL$VPN$WIFI$'\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
